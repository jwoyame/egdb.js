...
export class EditSession {
  ...
  static async start(
    ...
    // For now, we use the version's current state ID
    // In a full implementation, we might create a new child state
    const session = new EditSession(geodatabase, connection, version, version.stateId);
  ...
  /**
   * Save changes (commit to the version's state)
   * In this implementation, changes are written directly to the state,
   * so save() is a no-op confirmation that edits should be kept.
   */
  ...
  /**
   * Clean up A/D table entries for a specific registration ID
   * Deletes all rows with the current state ID
   */
  private async cleanupDeltaTables(registrationId: number): Promise<void> {
    const schema = await this.getSchemaForRegistrationId(registrationId);
    const schemaQuoted = this.quoteId(schema);
    const addsTable = `${schemaQuoted}.${this.quoteId(`a${registrationId}`)}`;
    const deletesTable = `${schemaQuoted}.${this.quoteId(`D${registrationId}`)}`;

    // Delete from A table (may affect 0 rows - that's fine)
    if (this.connection.driver === 'sqlserver') {
      await this.connection.execute(
        `DELETE FROM ${addsTable} WHERE SDE_STATE_ID = @p0`,
        [this.stateId]
      );
      await this.connection.execute(
        `DELETE FROM ${deletesTable} WHERE SDE_STATE_ID = @p0`,
        [this.stateId]
      );
    } else {
      // PostgreSQL uses lowercase column names
      await this.connection.execute(
        `DELETE FROM ${addsTable} WHERE sde_state_id = $1`,
        [this.stateId]
      );
      await this.connection.execute(
        `DELETE FROM ${deletesTable} WHERE sde_state_id = $1`,
        [this.stateId]
      );
    }
  }
   ...
   *
   * Removes all A/D table entries created with the current state ID.
   * If cleanup fails, the session remains open so the user can retry.
   *
  ...
  async discard(): Promise<void> {
    ...
    if (this.modifiedTables.size > 0) {
      // Check if already in a transaction (nested transaction not supported)
      ...
      try {
        for (const registrationId of this.modifiedTables) {
          await this.cleanupDeltaTables(registrationId);
        }
        ...
        // DO NOT set sessionState to 'discarded' - leave as 'open' so user can retry
        throw new Error(`Failed to discard changes: ${error instanceof Error ? error.message : error}`);
      }
    }
   ...
   * Calling close() while session is 'open' leaves changes in A/D tables.
   */
